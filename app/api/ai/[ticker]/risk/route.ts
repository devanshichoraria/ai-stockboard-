...
import { buildRiskPrompt } from "@/lib/prompts/risk"
...
export async function GET(
  ...
  try {
    const { ticker } = params
    const cacheKey = getCacheKey("ai-risk", ticker)
    ...
    const messages = buildRiskPrompt(profile.name, ticker, profile, metrics)
    ...
    return NextResponse.json(
      { error: "Failed to generate risk analysis", data: null },
      { status: 200 }
    )
  }
}
